...
///
/// Dependencies are injected so call sites stay DI-free. Route layer supplies
/// paths and [getCurrentUser] from the auth repository.
class OptionalSupabaseAuthGate extends StatefulWidget {
  const new({
  ...
  @override
  State<OptionalSupabaseAuthGate> createState() =>
      _OptionalSupabaseAuthGateState();
}

class _OptionalSupabaseAuthGateState extends State<OptionalSupabaseAuthGate> {
  ...
  @override
  void didUpdateWidget(covariant OptionalSupabaseAuthGate oldWidget) {
    ...
    _authStateSubscription = _disposables.trackSubscription(
      widget.authStateChanges.listen(
        ...
        onError: (Object error, StackTrace stackTrace) {
          AppLogger.error(
            'OptionalSupabaseAuthGate: auth state listener failed',
            error,
            stackTrace,
          );
        },
  ...
  void _checkAndRedirect(_) {
    ...
    try {
      ...
      if (!mounted) return;
      AppLogger.error(
        'OptionalSupabaseAuthGate: auth check failed, redirecting to fallback',
        error,
        stackTrace,
      );
