...
void main() {
      ...
      MaterialApp(
        ...
        home: Builder(
          builder: (final context) {
            return Scaffold(
              body: TextButton(onPressed: () => onOpen(context), child: const Text('Open picker')),
            );
          },
        ),
      ),
  ...
  group('showCountryPicker', () {
    testWidgets('returns selected country from Material bottom sheet', (final tester) async {
    ...
    testWidgets('shows Cupertino action sheet on iOS', (final tester) async {
      await pumpPickerHost(
        ...
        onOpen: (final context) =>
            showCountryPicker(context: context, selected: CountryOption.defaultCountry),
      );
