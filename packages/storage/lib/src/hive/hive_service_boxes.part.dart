...
mixin HiveServiceBoxOperations on Object {
  ...
  Future<Box<dynamic>> _openBoxInternal(
    ...
    try {
      ...
      if (!isRecoverableHiveFailure(error)) {
        AppLogger.error('Failed to open Hive box: $name', error, stackTrace);
        rethrow;
      }

      AppLogger.warning('Recovering corrupted Hive box: $name ($error)');
      try {
  ...
  Future<Box<dynamic>> _openBoxOnce(
    ...
    if (inMemoryDebug) {
      ...
      if (encrypted) {
        ...
        return Hive.openBox(name, bytes: bytes, encryptionCipher: cipher);
      }
  ...
  Future<void> closeBox(final String name) async {
    ...
    await self._mutexFor(name).run(() async {
      try {
        ...
        AppLogger.error('Failed to close Hive box: $name', error, stackTrace);
      }
    });
  }
  ...
  Future<void> deleteBox(final String name) async {
    ...
    await self._mutexFor(name).run(() async {
      try {
        ...
        AppLogger.error('Failed to delete Hive box: $name', error, stackTrace);
        rethrow;
      }
    });
  }
