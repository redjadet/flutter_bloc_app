...
///
/// Usage:
///   dart run tool/skill_budget_check.dart <inventory.json> [--enforce]
///
...
Future<void> main(List<String> args) async {
  if (args.isEmpty) {
    stderr.writeln('Usage: dart run tool/skill_budget_check.dart <inventory.json> [--enforce]');
    exitCode = 2;
    return;
  }
  ...
  final repoBudget = int.tryParse(Platform.environment['SKILL_BUDGET_REPO_TOKENS'] ?? '') ?? 12000;
  final cursorBudget =
      int.tryParse(Platform.environment['SKILL_BUDGET_CURSOR_TOKENS'] ?? '') ?? 120000;
  final maxSingleRepo =
      int.tryParse(Platform.environment['SKILL_BUDGET_MAX_SINGLE_REPO_TOKENS'] ?? '') ?? 3000;

  final inv = jsonDecode(await File(inventoryPath).readAsString()) as Map<String, Object?>;
  ...
  repoTop.sort((a, b) => (b['approxTokens'] as int).compareTo(a['approxTokens'] as int));
  ...
  final report = StringBuffer()
    ...
    ..writeln('- largest repoTemplates skill: $repoMax ($repoMaxPath) (max $maxSingleRepo)');

  stdout.writeln(report.toString().trimRight());

  final breaches = <String>[];
  if (sumRepo > repoBudget) breaches.add('repoTemplates sum $sumRepo > $repoBudget');
  if (sumCursor > cursorBudget) breaches.add('cursorSkills sum $sumCursor > $cursorBudget');
  if (repoMax > maxSingleRepo)
    breaches.add('repoTemplates max single $repoMax > $maxSingleRepo ($repoMaxPath)');
