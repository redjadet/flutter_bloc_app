...
for secrets_path in \
  ...
  apps/mobile/assets/config/secrets.json \
  .env \
  .env.local; do
  if ! git check-ignore -q --no-index "$secrets_path" 2>/dev/null; then
    finding_count=$((finding_count + 1))
    printf 'violation|%s:0:secrets_file_not_gitignored:local secret path must stay covered by .gitignore\n' "$secrets_path"
  fi
  if git ls-files --error-unmatch "$secrets_path" >/dev/null 2>&1; then
    finding_count=$((finding_count + 1))
    printf 'violation|%s:0:tracked_secrets_file:local secret file must stay gitignored\n' "$secrets_path"
  fi
done
