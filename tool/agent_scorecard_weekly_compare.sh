...
lines = [
    ...
    f"- p50 duration: `{round_duration_ms(cur['p50_duration_ms'])}ms`",
    "",
    ...
    f"- p50 duration: `{round_duration_ms(prev['p50_duration_ms'])}ms`",
    "",
    "## Delta (Current - Previous)",
    "",
    f"- Success rate delta: `{result['delta']['success_rate']:+.2%}`",
    f"- Delegate rate delta: `{result['delta']['delegate_rate']:+.2%}`",
    f"- p50 duration delta: `{round_duration_ms(result['delta']['p50_duration_ms']):+d}ms`",
