...
**Loop:** Plan → Execute → Verify → Report; **95% confident**; **Surgical diff**; **Report only after Verify**; **Self-verify final response**. **context ladder** → `docs/ai/context_loading.md`. **File verified reusable conclusions** in AKM. UI: `DESIGN.md` + `docs/design_system.md`. `./tool/check_agent_knowledge_base.sh` when agent docs change. No `./tool/request_codex_feedback.sh` unless user asks.
