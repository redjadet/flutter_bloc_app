...
- **Plan:** Goal / Context / Boundaries / Verification. Keep intent/spec/implementation separate; specs need evals, implementation follows repo seams. Non-trivial -> `tasks/cursor/todo.md`. Context ladder uses `docs/agent_project_context.md`; audit related landmines before feature/refactor. Delegation + hub matrix -> `agents-meta-behavior` + `docs/agent_knowledge_base.md#multi-agent-hub`. No edits until >=95% confident. One observe/revise loop. Prefer repo tools, code graph, browser, MCP over memory. Avoid giant prompts, context flooding, single-agent overload.
