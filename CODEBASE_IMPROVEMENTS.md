...
5. **Deep link failure telemetry** – `_consecutiveFailureCount` now increments when initialization/stream listeners fail, logging warnings at thresholds (3, 5, then every 5th failure thereafter) before a recover succeeds. It would be worth adding unit tests that stub `AppLogger.warning` (or expose a telemetry hook) to ensure these thresholds fire consistently, and keep an eye on log volume if a platform configuration issue floods the counter.
