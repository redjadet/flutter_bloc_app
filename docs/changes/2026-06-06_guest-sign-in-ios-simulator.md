...
- **Router:** Anonymous users may leave `/auth` after guest sign-in; Settings upgrade uses `AppRoutes.authUpgradePath()` (`?upgrade=true`) so anonymous users can return to auth only for upgrade (`apps/mobile/lib/app/router/auth_redirect.dart`, `apps/mobile/lib/core/router/app_routes.dart`, `apps/mobile/lib/core/router/app_routes_auth.part.dart`, `AccountSection`). Post-login redirect query params reject external/scheme tricks via `isSafeRedirectPath`.
...
echo "apps/mobile/lib/core/di/injector_helpers.dart" | python3 tool/integration_selective_resolve.py
```
