...
Execute Cursor context optimization plan: measure default surfaces, shrink always-on rules and repo-managed host skills, add local `.cursorignore`, refresh skill inventory. Detailed tables regenerate under `docs/audits/` **locally** ([audits README](../audits/README.md); JSON/snapshots are gitignored; this `docs/changes/` entry is the **committed** summary).

## What changed

- Local baseline + skill JSON under `docs/audits/` (see [audits README](../audits/README.md)): `skill_inventory_*.json`, `skill_rank_*.json` (regenerate with `dart run tool/skill_inventory.dart` / `dart run tool/skill_rank.dart`).
