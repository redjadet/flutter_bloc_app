...
| 3. Offline | Viewer-scoped Hive first-page cache + ordered mutation queue; pending local intent overlays remote until ack/reject. Each sync summary names every post with remaining queued mutations. | Cache-first reading and durable intent survive offline/recreation without clearing a same-post pending indicator after only one dispatch. Storage/reconciliation complexity is accepted. | Hive/queue/repository/Cubit tests. Re-plan before shared-account cache or production retention rules. |
