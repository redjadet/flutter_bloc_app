...
| --- | --- | --- |
| Secrets | Release/profile paths allow only public client configuration through `--dart-define` (from gitignored `.envrc` / `.env` via shell helpers); local debug may use provider keys, shared credentials stay server-side, and real platform config files stay gitignored. | [`security_and_secrets.md`](../security_and_secrets.md), [`tool/flutter_dart_defines_from_env.sh`](../../tool/flutter_dart_defines_from_env.sh), [`tool/load_env_file.sh`](../../tool/load_env_file.sh) |
| Local sensitive storage | `SecretStorage` wraps secure storage with device-bound Apple Keychain accessibility; Apple debug uses in-memory fallback because simulator Keychain is unreliable. | [`packages/app_shared_flutter/lib/src/platform/secure_secret_storage.dart`](../../packages/app_shared_flutter/lib/src/platform/secure_secret_storage.dart), [`security/storage_rules.md`](../security/storage_rules.md), [`engineering/apple_debug_hive_storage.md`](../engineering/apple_debug_hive_storage.md) |
...
| Backend caller auth | Render orchestration sends Firebase caller auth; FastAPI owns the Hugging Face credential and accepts no client provider-token header. | [`docs/integrations/render_fastapi_chat_demo.md`](../integrations/render_fastapi_chat_demo.md), [`apps/mobile/lib/features/chat/data/render_fastapi_chat_repository_send.part.dart`](../../apps/mobile/lib/features/chat/data/render_fastapi_chat_repository_send.part.dart) |
