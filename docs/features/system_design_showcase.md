...
| --- | --- | --- |
| Secrets | Only public client configuration flows through `--dart-define`; provider/shared credentials stay server-side, and real platform config files stay gitignored. | [`security_and_secrets.md`](../security_and_secrets.md), [`tool/flutter_dart_defines_from_env.sh`](../../tool/flutter_dart_defines_from_env.sh) |
