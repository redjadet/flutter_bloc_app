...
| ARCH-003 | Medium | Four features lack `<feature>.dart` barrel (`case_study_demo`, `staff_app_demo`, `library_demo` remain) | Inconsistent import surfaces | Add barrels on next touch; `igaming_demo.dart` shipped on branch | S | `test/features/igaming_demo/igaming_demo_barrel_test.dart` |
