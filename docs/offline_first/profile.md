...
1. Consider surfacing cache size + `lastSyncedAt` in end-user settings (dev/QA already covered).
2. Keep `ProfileCacheControlsSection` visible in settings to let users/QA clear stale snapshots (wired to `ProfileCacheRepository.clearProfile()`).
3. If profile ever supports edits, extend the repository to queue `SyncOperation`s and add conflict resolution similar to chat/counter.
