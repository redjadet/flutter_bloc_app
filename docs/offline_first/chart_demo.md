...
- **Firebase**
  ...
  - **Common failures**:
    - **Gen2 callable IAM**: if Cloud Run invoker is blocked, the client can see misleading `UNAUTHENTICATED`. (See [`firebase_setup.md`](../integrations/firebase_setup.md) for the required `gcloud run services add-iam-policy-binding ... allUsers` fix.)
