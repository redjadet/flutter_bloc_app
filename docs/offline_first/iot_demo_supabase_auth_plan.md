...
- In `lib/app/router/routes.dart` (or wherever IoT demo route is defined), do not render `IotDemoPage` directly. Instead, use a wrapper that:
...
- Update `docs/offline_first/iot_demo.md`: document auth requirement, per-user storage (box name pattern), and per-user RLS. Update “Implementation Status” when done.
