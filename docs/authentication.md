...
- **iOS simulator (debug):** Firebase App Check activation is skipped so anonymous sign-in can reach Firebase Auth without registering a debug token (`apps/mobile/lib/app/bootstrap/firebase_bootstrap_service_helpers.dart`, [Firebase setup](integrations/firebase_setup.md#troubleshooting)). Simulator **Runner** builds are code-signed (adhoc) with an **empty** entitlements plist — injecting `keychain-access-groups` causes `SBMainWorkspace` / `FBSOpenApplicationServiceErrorDomain` launch denial on current simulator runtimes. Signing alone restores Firebase Auth Keychain access. `apps/mobile/ios/scripts/sign_simulator_keychain_entitlements.sh` clears stale `.xcent` / `.xcent.der` before CodeSign. CocoaPods frameworks still embed via the unsigned rsync script. If Keychain still fails (`keychain-error`), DI falls back to a **local-only** guest session (`ios-simulator-debug-local-guest`) so the demo UI remains usable (`apps/mobile/lib/app/composition/features/register_auth_services.dart`). RTDB remotes stay wired on iOS simulator (unlike macOS debug). Physical iOS devices use real Firebase Auth and wired RTDB remotes. After entitlement/signing changes: quit `flutter run`, uninstall app from simulator if launch fails, full rebuild (hot reload is not enough).
