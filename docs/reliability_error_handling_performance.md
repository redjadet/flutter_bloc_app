...
| Concern | How it’s handled | Where to read more |
| -------- | ------------------- | -------------------- |
| **Large JSON** | [isolate_json](../apps/mobile/lib/app/utils/isolate_json.dart): `decodeJsonMap` / `decodeJsonList` / `encodeJsonIsolate` for payloads &gt;8KB (configurable). Prefer **`decodeJsonMapFromBytes`** / **`decodeJsonListFromBytes`** when the HTTP layer exposes UTF-8 **`List<int>`** (e.g. Dio `ResponseType.bytes`) so parsing does not allocate a full body **`String`** first. No raw `jsonDecode`/`jsonEncode` for large data (enforced by [tool/check_raw_json_decode.sh](../tool/check_raw_json_decode.sh)). | [compute_isolate_review.md](performance/compute_isolate_review.md); [changes/2026-05-14_json_utf8_bytes_decode_and_hf_bytes.md](changes/2026-05-14_json_utf8_bytes_decode_and_hf_bytes.md) |
