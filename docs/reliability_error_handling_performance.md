...
| -------- | ------------------- | -------------------- |
| **HTTP retries** | [ResilientHttpClient](../lib/shared/http/resilient_http_client.dart): automatic retries for transient errors, 401 token refresh, network check before send, telemetry. For concurrent 401s, refresh is serialized in [AuthTokenManager](../lib/shared/http/auth_token_manager.dart) (single-flight), so callers share one refresh and retry with refreshed bearer tokens. [resilient_http_client_extensions](../lib/shared/http/resilient_http_client_extensions.dart) getMapped/postMapped throw [HttpRequestFailure](../lib/shared/utils/http_request_failure.dart) for status ≥ 400 so callers get statusCode. Retry delay aligned with [RetryPolicy](../lib/shared/utils/retry_policy.dart) (exponential + jitter). | [SHARED_UTILITIES.md](SHARED_UTILITIES.md) – “Reliability and retries” |
...
| **In-flight / request-id guards** | Cubits that trigger load (chart, graphql, profile, todo list, search) use request-id checks to ignore stale completions. Repository refresh paths also use in-flight coalescing (`Future` gates) so concurrent callers share one runner (search/profile/remote config). `BackgroundSyncCoordinator.flush()` coalesces immediate triggers and avoids overlapping sync cycles while allowing one follow-up run when another immediate trigger arrives mid-sync. | Race-condition prevention; [SearchCubit](../lib/features/search/presentation/search_cubit.dart), [OfflineFirstSearchRepository](../lib/features/search/data/offline_first_search_repository.dart), [OfflineFirstProfileRepository](../lib/features/profile/data/offline_first_profile_repository.dart), [OfflineFirstRemoteConfigRepository](../lib/features/remote_config/data/offline_first_remote_config_repository.dart), [BackgroundSyncCoordinator](../lib/shared/sync/background_sync_coordinator.dart) |
