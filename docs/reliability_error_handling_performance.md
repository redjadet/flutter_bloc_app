...
| **In-flight / request-id guards** | **Request-id:** [RequestIdGuard](https://pub.dev/packages/ilkersevim_async_utils) — use `next()` before starting a load and `isCurrent(id)` before emitting so stale completions are ignored. After a **successful mutation**, if the guard is inactive (superseded by a newer read), still return success (`true` for `Future<bool>`, bare `return` for `Future<void>`)—never `return false`, which surfaces false failure UI. Enforced by `tool/check_mutation_success_after_guard.sh`. **In-flight coalescing:** [InFlightCoalescer](https://pub.dev/packages/ilkersevim_async_utils) (single) and [KeyedInFlightCoalescer](https://pub.dev/packages/ilkersevim_async_utils) (per-key) so concurrent callers share one run. Used in cubits (chart, graphql, profile, todo list, search, camera) and repos (search, profile, remote config). `BackgroundSyncCoordinator.flush()` coalesces immediate triggers and avoids overlapping sync cycles. | Race-condition prevention; [reliability_error_handling_performance.md](reliability_error_handling_performance.md) §1 |
| **Subscriptions, timers, and lifecycle** | [DisposableBag](https://pub.dev/packages/ilkersevim_disposables) is the shared lifecycle primitive for cancellable/closable resources. [CubitSubscriptionMixin](../apps/mobile/lib/app/utils/bloc/cubit_subscription_mixin.dart) centralizes subscription cancellation **and TimerService handle disposal** (`registerSubscription`, `registerTimer`) on `close()`. [SubscriptionManager](https://pub.dev/packages/ilkersevim_disposables) and [TimerHandleManager](https://pub.dev/packages/ilkersevim_disposables) are thin, bounded facades for repositories/services (including delayed restart timers) that delegate cleanup behavior to the same primitive. Provide `onError` when calling `stream.listen(...)` in widgets/adapters so errors are logged. | [REPOSITORY_LIFECYCLE.md](engineering/REPOSITORY_LIFECYCLE.md) |
...
| **Cubit async errors** | [CubitExceptionHandler](../apps/mobile/lib/app/utils/cubit_async_operations.dart) with `isAlive: () => !isClosed` so callbacks don’t run after close. For [HttpRequestFailure](../packages/utilities/lib/src/errors/http_request_failure.dart), the handler uses NetworkErrorMapper so emitted messages are status-aware. | State, Lifecycle, and Async Safety; [REPOSITORY_LIFECYCLE.md](engineering/REPOSITORY_LIFECYCLE.md) |
| **Structured error codes** | Enum + mapper ownership: [observability.md](observability.md) § Structured error codes (`AppErrorCode`, `NetworkErrorMapper.getErrorCode`). | [observability.md](observability.md) |
...
| **Crash reporting** | Firebase Crashlytics when Firebase initializes (`FirebaseCrashlyticsBootstrap.registerHandlers`); no PII in reports. Inactive when Firebase is off (tests/mock harness). | [observability.md](observability.md); [firebase_crashlytics_bootstrap.dart](../apps/mobile/lib/app/bootstrap/firebase_crashlytics_bootstrap.dart) |
...
| Concern | How it’s handled | Where to read more |
| -------- | ------------------- | -------------------- |
| **Large JSON** | [ilkersevim_json_isolate](https://pub.dev/packages/ilkersevim_json_isolate): `decodeJsonMap` / `decodeJsonList` / `encodeJsonIsolate` for payloads &gt;8KB (configurable). Prefer **`decodeJsonMapFromBytes`** / **`decodeJsonListFromBytes`** when the HTTP layer exposes UTF-8 **`List<int>`** (e.g. Dio `ResponseType.bytes`) so parsing does not allocate a full body **`String`** first. No raw `jsonDecode`/`jsonEncode` for large data (enforced by [tool/check_raw_json_decode.sh](../tool/check_raw_json_decode.sh)). | [compute_isolate_review.md](performance/compute_isolate_review.md); [changes/2026-05-14_json_utf8_bytes_decode_and_hf_bytes.md](changes/2026-05-14_json_utf8_bytes_decode_and_hf_bytes.md) |
| **List and scroll** | Guidelines: `RepaintBoundary` for heavy list items; `CustomScrollView` + slivers over shrinkWrap for long lists; builder patterns for dynamic length. | [performance_bottlenecks.md](performance/performance_bottlenecks.md) – “List and scroll performance” |
...
| **Memory and caches** | Documented approach: trim in-memory caches (search, profile, image, feature static caches) on memory pressure via a single entry point; optional for enterprise/high-traffic. Dart GC frees unreachable heap objects eventually — dispose/trim still required ([dart_memory_under_the_hood.md](performance/dart_memory_under_the_hood.md)). | [REPOSITORY_LIFECYCLE.md](engineering/REPOSITORY_LIFECYCLE.md) – “Memory pressure and cache trim”; [memory_management.md](performance/memory_management.md) |
...
| Area | Files |
| ------ | -------- |
| HTTP / retries | [app_dio.dart](../apps/mobile/lib/app/http/app_dio.dart), [interceptors/](../packages/networking/lib/src/interceptors/) (auth, retry, network check, telemetry), [network_guard.dart](../packages/networking/lib/src/guards/network_guard.dart), [ilkersevim_retry](https://pub.dev/packages/ilkersevim_retry) |
...
| Async / lifecycle | [cubit_async_operations.dart](../apps/mobile/lib/app/utils/cubit_async_operations.dart), [cubit_subscription_mixin.dart](../apps/mobile/lib/app/utils/bloc/cubit_subscription_mixin.dart), [subscription_manager.dart](https://pub.dev/packages/ilkersevim_disposables) |
| Performance | [ilkersevim_json_isolate](https://pub.dev/packages/ilkersevim_json_isolate), [TimerService](../packages/core/lib/src/time/timer_service.dart) |
