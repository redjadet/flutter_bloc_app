...
| Auth session lifecycle races | `test/app/presentation/cubit/app_auth_cubit_test.dart`, `test/core/auth/session_lifecycle_coordinator_test.dart` (+ `tool/check_regression_guards.sh` route for `lib/core/auth/*` and `AppAuthCubit`) |
