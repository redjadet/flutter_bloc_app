...
| Apple Keychain dual-store (legacy peek + delete clears both) | `test/secure_secret_storage_test.dart` (+ `tool/check_keychain_dual_store_symmetry.sh`, `tool/check_apple_debug_hive_storage.sh`) |
| Hive per-box mutex / getBox RMW lost-update | `test/features/notes_demo/data/hive_notes_repository_test.dart` (`concurrent save and delete…`); `test/features/todo_list/data/hive_todo_repository_test.dart` (same); `packages/storage/test/sync/pending_sync_repository_test.dart` (`concurrent enqueue…`) (+ `tool/check_hive_getbox_rmw.sh`) |
