...
| Don't-overwrite stale sync | `test/features/counter/data/offline_first_counter_repository_test.dart`, `test/features/todo_list/data/offline_first_todo_repository_test.dart` (+ `tool/check_offline_first_remote_merge.sh`). Include timestamp gates (`does not overwrite newer …`), queue replay (`does not push stale pending …`), TOCTOU re-read tests (`re-checks local before save` / `… before deleting`), and remote fetch failure retention (`does not delete local … when remote fetch fails` / `does not overwrite local when remote load fails`). |
