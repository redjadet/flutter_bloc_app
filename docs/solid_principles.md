...
- All services are wired through `getIt` (feature-specific registration files in `apps/mobile/lib/app/composition/`), depending on interfaces (e.g., `NetworkStatusService`, `TimerService`, repositories) rather than concrete classes.
...
- Keep domain types Flutter-agnostic; platform/UI utilities stay under `apps/mobile/lib/app/`.
...
- DI registrations bind interfaces to implementations in feature-specific files under `apps/mobile/lib/app/composition/` (e.g., `register_chat_services.dart`, `register_profile_services.dart`).
