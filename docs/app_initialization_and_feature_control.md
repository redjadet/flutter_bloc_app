...
- **Bootstrap:** [apps/mobile/lib/core/bootstrap/bootstrap_coordinator.dart](../apps/mobile/lib/core/bootstrap/bootstrap_coordinator.dart) sets flavor, runs platform init, secrets, Firebase, Supabase (when `SUPABASE_URL` and `SUPABASE_ANON_KEY` are present; used by the IoT demo backend and the optional Supabase Auth page; when not present, the IoT demo remains accessible in local-only mode), DI, migrations, and materializes [AppRuntimeConfig](../apps/mobile/lib/core/config/app_runtime_config.dart) and [BackendAvailability](../apps/mobile/lib/core/config/backend_availability.dart) before `runApp`.
- **Runtime config:** [AppRuntimeConfig](../apps/mobile/lib/core/config/app_runtime_config.dart) holds flavor, optional `apiBaseUrl` (from `--dart-define=API_BASE_URL`), and `skeletonDelay`; built via `AppRuntimeConfig.fromBootstrap()` and registered in DI. Features and repositories (e.g. counter route, [DelayedChartRepository](../apps/mobile/lib/features/chart/data/delayed_chart_repository.dart)) read from it for init-controlled behavior.
- **Web no-backend policy:** [BackendAvailability](../apps/mobile/lib/core/config/backend_availability.dart) is registered at bootstrap and gates web-only guest auth, chat local fallback, and Supabase session bypass for Chat/IoT demos when Firebase/Supabase are absent. See [changes/2026-06-17_web-no-backend-mode.md](changes/2026-06-17_web-no-backend-mode.md).
