...
- **`agent_memory_auto_maintain.sh`**: Safe automatic upkeep — `--verify` (invariants; runs after `sync_agent_assets.sh --apply`), `--if-changed` / `--fix-links` (local-only markdown link normalize on agent-scope paths; invoked from `check_agent_knowledge_base.sh`). No compress/trim/host writes. Opt-out: `AGENT_MEMORY_AUTO_MAINTAIN=0`.
