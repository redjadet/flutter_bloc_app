...
- **`check_aidlc_artifacts.sh`**: Validates AIDLC Lite tracker YAML and T2 run bundles (schema v1, extensions, gate_events, active-run rule); `--self-test` runs `tool/fixtures/aidlc_artifacts`.
...
- **`run_memory_leak_tests.sh`**: Discovers test files calling `leakSafeTestWidgets` and runs only that tagged suite. The checklist runs it by default (`CHECKLIST_RUN_MEMORY_LEAK_TESTS=1`). Docs: [`../performance/memory_testing.md`](../performance/memory_testing.md).
...
**Router companion inside checklist:** After static checks, `./bin/checklist` may run `./bin/router_feature_validate` when changed files match [`.cursor/rules/router-feature-validation.mdc`](../../tool/agent_host_templates/cursor/rules/router-feature-validation.mdc) globs (same rules as `tool/check_router_trigger_precision.sh`). Expect extra time when router/auth UI changes. Skip locally: `CHECKLIST_SKIP_ROUTER_VALIDATE=1`.
...
- **`check_no_isolate_run_in_presentation.sh`**: Prevents `Isolate.run` under app/package `presentation/**` paths. Closures from `State`/widgets often capture non-sendable Flutter objects and crash with *illegal argument in isolate message*; use `compute(topLevelOrStaticCallback, message)` from `package:flutter/foundation.dart` instead (see `package:ilkersevim_json_isolate`). The check fails closed when expected source roots are absent; use `--paths PATH...` for focused fixture or changed-path proof. Suppress with `check-ignore` on same or previous line only for rare, reviewed cases.
