...
- **`run_file_length_lint.sh`**: Fail when `dart analyze --format machine .` reports `FILE_TOO_LONG` under `lib/` (native `file_length_lint` plugin via `plugins.file_length_lint.path`; max 225 **physical** lines). Exits 1 on analyzer plugin failures. Skipped with `SKIP_FILE_LENGTH_LINT=1` or `CHECKLIST_RUN_FILE_LENGTH_LINT=0`. Also surfaced by `flutter analyze` when the plugin is enabled.
- **`run_file_length_lint_test.py`**: Regression harness for plugin wiring (`python3 tool/run_file_length_lint_test.py`). Probes must use physical newlines (`wc -l` ≥ 226), not many `//` tokens on one line. Checklist auto-runs when `custom_lints/file_length_lint/**`, `analysis_options.yaml`, or the lint scripts change (`CHECKLIST_RUN_FILE_LENGTH_LINT_INTEGRATION_TEST=auto|0|1`).
