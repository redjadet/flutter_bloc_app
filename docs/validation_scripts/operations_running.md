...
- **`prune_git_stale.sh`** / **`./bin/prune-git-stale`**: Full stale prune — ancestor **worktrees**, **local + remote** topics that are true-merged into the base, `[gone]`, or **squash-merged** (batched `gh pr list --state merged`). Optional **`--closed-prs`** deletes CLOSED unmerged heads; **`--keep NAME`** (repeatable) protects exceptions. Batches `git push --delete`. Defaults to **dry-run**; pass **`--apply`** to execute. See `bash tool/prune_git_stale.sh --help`.
