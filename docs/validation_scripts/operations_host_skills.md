...
By default this **excludes** `~/.cursor/plugins/cache` (marketplace vendor skills) so budget/rank reports focus on repo + `~/.cursor/skills` + `~/.agents/skills`. Opt in to scan vendor cache:

```bash
dart run tool/skill_inventory.dart docs/audits/skill_inventory_latest.json --include-plugin-cache
```

Vendor-only rollup (what to disable under **Cursor Settings → Plugins**):

```bash
bash tool/audit_vendor_plugin_skills.sh docs/audits/vendor_plugin_inventory_latest.json
```

**Cursor Plugins checklist (monthly):** `audit_vendor_plugin_skills.sh` → disable unused plugins in **Cursor → Settings → Plugins** (prefer slugs with `recommendedForFlutter: false` in the audit JSON) → `dart run tool/skill_inventory.dart docs/audits/skill_inventory_latest.json` → `check_skill_budgets.sh … report` → reload Cursor.

### Suggested habits

Canonical list for agent/host context hygiene (see also [`docs/audits/README.md`](../audits/README.md) § Habits).

| When | Do |
| --- | --- |
| After global skill **install/update** or manual archive under `~/.agents/skills` | Regenerate `skill_inventory_latest.json`, then `check_skill_budgets.sh … report` — stale inventory **overstates** `agentsSkills` |
| After enabling/disabling **Cursor marketplace plugins** | `bash tool/audit_vendor_plugin_skills.sh` (vendor load is **not** in editable inventory) |
| After `trim_duplicate_agent_skills.sh --apply` | Reload Cursor; regen inventory before trusting the next budget report |
| Monthly or after plugin churn | Run the **Cursor Plugins checklist** (above) |
| `agentsSkills` budget breach | `trim_duplicate_agent_skills.sh` (dry-run) → `--apply` only if plan is intentional → regen inventory → reload Cursor |
| Non-trivial task **finish** (docs/host/tooling in scope) | `./bin/agent-maintain closeout` |
| Edit under `tool/agent_host_templates/**` | `./bin/agent-maintain after-host-edit` same turn, then reload Cursor |
| Routine host upkeep (no template edit) | `./bin/agent-maintain routine` (preview) or `routine --apply` when operator wants sync + budget report |

Do **not** run `trim --apply` on a schedule while budgets are green — use rank/inventory to decide.
