...
- **Styles:** Use `AppStyles.card`, `AppStyles.profileOutlinedButton`, `AppStyles.listTile`, `AppStyles.inputField`, `AppStyles.appBar`, `AppStyles.chip`, `AppStyles.dialogContent` from `app_styles.dart` in widgets (e.g. `Box(style: AppStyles.card, child: ...)`). Card uses a dark-mode variant (`$on.dark`); listTile uses a breakpoint variant (`$on.medium` for horizontal padding). Example migration: **GraphqlDataSourceBadge** uses `AppStyles.chip`. Prefer tokens and these styles for new UI so styling stays consistent.
