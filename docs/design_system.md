...
- **Styles:** Use `AppStyles.card`, `AppStyles.profileOutlinedButton`, `AppStyles.listTile`, `AppStyles.inputField`, `AppStyles.appBar`, `AppStyles.chip`, `AppStyles.dialogContent` from `app_styles.dart` in widgets (e.g. `Box(style: AppStyles.card, child: ...)`). Card uses a dark-mode variant (`$on.dark`); listTile uses a breakpoint variant (`$on.medium` for horizontal padding). Example migrations: **GraphqlDataSourceBadge**, **GraphqlCountryCard** `_DetailChip`, and **SyncDiagnosticsSection** chips use `AppStyles.chip`; **CommonCard** is used by settings sections, **SettingsCard**, **CalculatorSummaryCard**, **WordCard**, **GraphqlCountryCard**; **PlatformAdaptiveSheets.showPickerModal** (Material) uses `AppStyles.dialogContent`. Prefer tokens and these styles for new UI so styling stays consistent.
- **Tests:** Widget tests that need Mix theme can use `pumpWithMixTheme(tester, child: ...)` from `test/helpers/pump_with_mix_theme.dart`.
