...
| Golden / visual regression | Golden tests for shared design-system widgets or stable visual contracts; update with `flutter test --update-goldens` after intentional visual change or Flutter upgrade | New/changed shared widget in `apps/mobile/lib/app/`, theme token, or typography affecting goldens |
