...
- **Android Play Store (Fastlane):** Put the same variable names in gitignored `.env.android.release` (template: [`.env.android.release.example`](../../.env.android.release.example)). [`tool/release_android_play.sh`](../../tool/release_android_play.sh) sources that file before Fastlane; the Android build lane uses the same `flutter_dart_defines_from_env.sh` helper, so optional `CHAT_FASTAPICLOUD_*` / `CHAT_RENDER_*` behave like `.envrc` for compile-time defines. See [Android Play Store release SOP](../android_play_store_release_sop.md).
