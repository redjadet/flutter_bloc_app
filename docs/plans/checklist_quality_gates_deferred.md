...
| **QG-D05** | Navigation / perf | `check_deferred_heavy_routes.sh` | defer | Five deferred routes documented in baseline; need allowlist vs heuristic policy before failing CI. | Allowlist table matches [route manifest](checklist_quality_gates_baseline.md#deferred-route-manifest-m0-appendix); one `./bin/checklist` run with gate wired shows 0 violations on `main`. |
