...
| **QG-D03** | Rebuild | `check_bloc_rebuild_scoping.sh` | defer | `BlocBuilder` without `buildWhen` is common in demos; false-positive rate unknown without fixture matrix. | Fixture trio (good/bad/suppressed); FP rate &lt; agreed threshold on `apps/mobile/lib/features/**/presentation/**` sample; owner accepts fix churn count. |
