...
| **QG-D05** | Navigation / perf | `check_deferred_heavy_routes.sh` | **promoted (warn)** | Shipped 2026-06-03: allowlist `route_groups.dart` + `routes_core.dart`; default `CHECK_DEFERRED_HEAVY_ROUTES_MODE=warn`. | Flip to fail when a second deferred route file is intentional; keep fixtures under `tool/fixtures/deferred_heavy_routes/`. |
...
| **QG-D07** | Lifecycle | `check_lifecycle_observer_dispose.sh` | **promoted (warn)** | Shipped 2026-06-03: scans `lib/` for `addObserver(this)` without `removeObserver(this)` in dispose; default warn. | Flip to fail if a new observer site lands without dispose; fixtures under `tool/fixtures/lifecycle_observer_dispose/`. |
