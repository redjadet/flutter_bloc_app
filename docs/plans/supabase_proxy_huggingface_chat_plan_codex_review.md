...
- Expand `## Code touchpoints` with a `### File map` subsection.
  - List likely files to add/update, not just directories, for example: `apps/mobile/lib/features/chat/data/...`, `apps/mobile/lib/features/chat/domain/...`, `apps/mobile/lib/core/di/features/register_chat_services.dart`, `apps/mobile/lib/features/chat/presentation/pages/chat_page.dart`, `apps/mobile/lib/features/chat/presentation/widgets/...`, `supabase/functions/<name>/index.ts`, `supabase/config.toml`, [`supabase/README.md`](../../supabase/README.md).
