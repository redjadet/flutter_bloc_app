...
- **Remaining:** mix_lint 2.x (vendored `custom_lints/mix_lint`, `analysis_server_plugin`) runs via native `plugins:` in `analysis_options.yaml` and `./tool/run_mix_lint.sh`. Incremental migration continues when touching other screens.
...
- [x] `./bin/checklist` now also runs `mix_lint` and `file_length_lint` via `tool/run_mix_lint.sh` / `tool/run_file_length_lint.sh` (auto when Dart/analyzer-relevant changes).
...
- [x] Evaluate mix_lint: vendored `custom_lints/mix_lint` 2.0 (`analysis_server_plugin`, analyzer 10). Run with `./tool/run_mix_lint.sh` or `dart analyze` (plugin rules). Rules: variants last, avoid tokens in style/scope, max attributes per style, prefer dot shorthands, mixable styler has create.
...
- Create a **mix theme module** (e.g. under `apps/mobile/lib/app/theme/` or `packages/design_system/`) that:
  - Builds `MixScope` token values from:
    ...
    - **Spaces:** Map `UI` (e.g. `gapS`, `gapM`, `cardPadH`) and, if needed, responsive values from [packages/design_system/lib/src/responsive/responsive_layout.dart](../packages/design_system/lib/src/responsive/responsive_layout.dart) into mix `space` tokens.
...
- Add a **central style module** (e.g. `packages/design_system/lib/src/styles/app_styles.dart` or under `apps/mobile/lib/app/theme/mix/`) that defines:
  ...
  - **Context variants** where useful:
    ...
    - If mix supports breakpoint-based context variants, mirror `isMobile`/`isTablet`/`isDesktop` (e.g. different padding or font size) so responsive behavior stays consistent with [packages/design_system/lib/src/responsive/responsive_layout.dart](../packages/design_system/lib/src/responsive/responsive_layout.dart).
