...
| RISK-ENGINEERING-SCORE-DROP | Engineering scorecard wiring breaks; agents conflate harness with engineering quality | [`../engineering/engineering_quality_scorecard.md`](../engineering/engineering_quality_scorecard.md); update badge via `bash ../../tool/update_engineering_quality_badge.sh`; run closeout when engineering scope | `bash ../../tool/check_engineering_quality_scorecard_gate.sh`; `bash ../../tool/update_engineering_quality_badge.sh --check`; `scope_has_engineering_edits` in closeout | Fix owner docs/scripts/needles; update change note; rerun closeout |
...
| RISK-SCOPE-CREEP | Agent expands beyond user-owned write-set, performs unrelated cleanup, or stalls safe work with permission loops | Declare write-set/boundaries; use `SAFETY-01`; autonomously complete routine reversible repo-local work; separate observations from requested changes | Review changed paths/diff against request; `bash ../../tool/check_agent_safety_contracts.sh` | Return to requested outcome; revert only agent-authored out-of-scope changes after approval when destructive; retain/report user work |
...
| RISK-VALIDATION-SHORTCUT | Agent reports success from reused or partial checks when fresh proof is needed | [`agent_operating_manual.md`](agent_operating_manual.md) AI behaviour rules; scorecard gate in [`harness_scorecard.md`](harness_scorecard.md) | `bash ../../tool/check_harness_scorecard_gate.sh`; `./bin/checklist-fast --no-reuse` or `./bin/checklist`; `git diff --check` | Rerun correct lane; report skipped checks honestly |
| RISK-AIDLC-LIFECYCLE-DRIFT | Stale stage, missing approve/continue, unrecorded scope change, or complete without reconciliation/proof | [`aidlc_workflow.md`](aidlc_workflow.md); [`../engineering/aidlc_artifact_contract.md`](../engineering/aidlc_artifact_contract.md); record `invalidate` on material change | `bash ../../tool/check_aidlc_artifacts.sh`; tracker AIDLC section review; `bash ../../tool/validate_task_trackers.sh` | Invalidate downstream approvals; return to owning stage; new approve/continue; rerun routed proof |
