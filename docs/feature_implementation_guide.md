...
- **Mapbox demo:** `lib/features/mapbox_demo/` is lazy-loaded as a separate route (`/mapbox-maps`) using `mapbox_maps_flutter`. It reads `MAPBOX_ACCESS_TOKEN` via `SecretConfig` and gates unsupported platforms/token at the page level.
...
| Maps (Google + Mapbox) | Implemented (demo) | `/google-maps`, `/mapbox-maps` | google_maps_integration.md, feature_implementation_guide.md#12-maps |
