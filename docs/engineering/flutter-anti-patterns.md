...
| --- | --- | --- | --- | --- |
| AP-01 | Feature `data` imports another feature's `domain` | Hidden coupling; breaks modular monolith | Narrow port in a package or app composition (or owning feature `domain` port); wire in DI | **Fixed (chat)** — `RemoteBackendAuthPort`; `modular_metrics --cross-feature-only` empty. Case study still uses port via same seam |
