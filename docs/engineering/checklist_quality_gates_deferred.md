...
| **QG-D03** | Rebuild | `check_bloc_rebuild_scoping.sh` | **ADR-deferred** | `BlocBuilder` without `buildWhen` is common in demos; false-positive rate unknown without analyzer ruleset. Non-blocking for Engineering scorecard. | Owner: [`docs/adr/0005-interview-showcase-scope.md`](../adr/0005-interview-showcase-scope.md) § quality-gate promotion follow-up; fixture trio + FP rate threshold before promotion. |
