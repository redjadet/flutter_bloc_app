...
Use `TimerService` (from `packages/core/lib/src/time/timer_service.dart`) for any delayed work where:
...
- **Repositories/services**: Prefer `TimerHandleManager` (`packages/utilities/lib/src/timers/timer_handle_manager.dart`) to keep timer handles bounded and to ensure delayed restarts don’t outlive `dispose()`.
