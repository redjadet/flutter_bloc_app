...
| Memory / lifecycle | proof | `bash tool/run_memory_lint.sh` && `bash tool/run_memory_leak_tests.sh` | both exit 0; checklist defaults enforce both gates |
...
| Quality gates honesty | 10/10 | `bash tool/check_engineering_quality_scorecard_gate.sh` | [`checklist_quality_gates_deferred.md`](checklist_quality_gates_deferred.md) open backlog has no bare `defer` rows (`open (warn)` / `reject` / `ADR-deferred`); shipped fail gates stay in `CHECK_SCRIPTS`; baseline post-MVP status matches |
