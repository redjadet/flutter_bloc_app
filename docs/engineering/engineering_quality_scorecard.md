...
| Quality gates honesty | 10/10 | `bash tool/check_engineering_quality_scorecard_gate.sh` | [`checklist_quality_gates_deferred.md`](checklist_quality_gates_deferred.md) has no bare `defer` rows: every item is `promoted`, `reject`, or `ADR-deferred` with an owning link |
