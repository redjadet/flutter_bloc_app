...
- `apps/mobile/lib/app.dart` router refresh/auth wiring
...
| --- | --- |
| Docs/tooling only (e.g. `docs/**`, `ai/**`, `llms.txt`, `tool/*.sh`, `bin/*`, `.cursor/**`) | `./bin/checklist-fast` + `bash tool/check_agent_knowledge_base.sh`; agents: `./bin/agent-maintain closeout` before claiming done |
...
| UI/design brief or design-system code ([`DESIGN.md`](../../DESIGN.md), [`design_system.md`](../design_system.md), `apps/mobile/lib/app/theme/**`, `packages/design_system/**`) | `./tool/check_design_md.sh` when [`DESIGN.md`](../../DESIGN.md) changed; `./tool/run_mix_lint.sh` when Mix tokens/styles changed; focused widget/app-visible proof when runtime UI changed |
