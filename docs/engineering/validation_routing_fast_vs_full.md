...
| Presentation navigation in domain/data, presentation `dart:io` *Sync | `./bin/checklist` (fail: `check_navigation_outside_presentation.sh`, `check_sync_io_in_presentation.sh`; warn: `check_remote_image_cache_hints.sh`, `check_cubit_subscription_cancel.sh`). Theme map: `CHECKLIST_EXPLAIN_THEMES=1`. Deferred gates: [`plans/checklist_quality_gates_deferred.md`](../plans/checklist_quality_gates_deferred.md). |
