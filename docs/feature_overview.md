...
| Social feed demo (simulated) | `/social-feed-demo` (Example hub entry) | `apps/mobile/lib/features/social_feed_demo/` | Credential-free senior-signal feed: cursor paging, offline queue, optimistic like/comment, realtime banner, viewer isolation. Doc: [`features/social_feed_demo.md`](features/social_feed_demo.md). |
